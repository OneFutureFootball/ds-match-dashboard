...
match_file <- match_file %>%
    ...
    mutate(
        ...
        prev_x = lag(ball_x),
        prev_y = lag(ball_y),
        prev_x = ifelse(state%in%c('Kickoff','Throw In','Corner', 'Free Kick', 'Keeper Possession', 'Goal Kick')|prev_outcome=='post'|prev_outcome=='blocked'|(prev_outcome=='saved' & position!='GK'),NA_real_,prev_x),
        prev_y = ifelse(state%in%c('Kickoff','Throw In','Corner', 'Free Kick', 'Keeper Possession', 'Goal Kick')|prev_outcome=='post'|prev_outcome=='blocked'|(prev_outcome=='saved' & position!='GK'),NA_real_,prev_y),
        ...
        prev_x2 = ifelse(prev_state%in%c('Kickoff','Throw In','Corner', 'Free Kick', 'Keeper Possession', 'Goal Kick')|prev_outcome2=='blocked'|prev_outcome2=='post'|(prev_outcome2=='saved' & prev_position!='GK'),NA_real_,prev_x2),
        prev_y2 = ifelse(prev_state%in%c('Kickoff','Throw In','Corner', 'Free Kick', 'Keeper Possession', 'Goal Kick')|prev_outcome2=='blocked'|prev_outcome2=='post'|(prev_outcome2=='saved' & prev_position!='GK'),NA_real_,prev_y2),
        next_x = case_when(
...
key_moments <- match_file %>% 
    ...
    mutate(
        oth_short_player = sapply(oth_full_name,shorten_name),
        short_player = sapply(full_name,shorten_name),
        live_label = case_when(
            action=='KICKOFF' ~ action,
            action=='SHOT' ~ 'SHOT',
            state=='Substitution' & oth_role=='injury' ~ 'INJURY',
            state=='Substitution' ~ paste0(' > ',toupper(case_when(nchar(oth_full_name)>=20 ~ oth_short_player,
                                                                   TRUE ~ oth_full_name))),
            state=='Corner' ~ 'CORNER',
        ...
        full_name = case_when(
            ...
            state=='Substitution' & nchar(full_name)>=20 ~ short_player,
            TRUE ~ full_name
        ),
        position = case_when(
